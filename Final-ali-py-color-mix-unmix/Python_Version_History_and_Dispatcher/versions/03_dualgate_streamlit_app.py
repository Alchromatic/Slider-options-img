...
# ---- Optional dual residual gate sidecar (benchmark-tuned) ----
_DUAL_GATE_IMPORT_ERROR: Optional[str] = None
DUAL_GATE_AVAILABLE = False
dual_gate_mod: Optional[Any] = None

def _load_dual_gate_module() -> Any:
    here = Path(__file__).resolve().parent
    src = here / "miniapp_dual_residual_gate.py"
    if not src.exists():
        raise FileNotFoundError(f"Dual residual gate module not found: {src}")
    spec = importlib.util.spec_from_file_location("miniapp_dual_residual_gate_src", str(src))
...
try:
    dual_gate_mod = _load_dual_gate_module()
    DUAL_GATE_AVAILABLE = True
except Exception as _e:
    _DUAL_GATE_IMPORT_ERROR = str(_e)
    dual_gate_mod = None

def dual_gate_bundle_known_hexes(bundle: Optional[Dict[str, Any]]) -> List[str]:
    if not isinstance(bundle, dict):
        return []
    schema = bundle.get("base_feature_schema", {})
    if not isinstance(schema, dict):
        return []
    out: List[str] = []
    for h in schema.get("known_hexes", []) or []:
        hh = clean_hex(str(h))
        if hh:
            out.append(hh)
    return sorted(set(out))

def dual_gate_allows_hexes(bundle: Optional[Dict[str, Any]], hexes: Sequence[str]) -> bool:
    known = set(dual_gate_bundle_known_hexes(bundle))
    if not known:
        return True
    hx = set()
    for h in hexes:
...
def predict_lin_from_bases_lin(
    ...
    nn_scale: float = 0.25,
    pigment_hexes: Optional[Sequence[str]] = None,
    dual_gate_bundle: Optional[Dict[str, Any]] = None,
) -> Tuple[float, float, float]:
    """
    ...
    - If dual_gate_bundle is provided, it acts as a benchmark-tuned post-mix sidecar on top
      of the locked Hybrid (KM ⊕ Linear) client profile and therefore bypasses the calibrator,
      NN residual, and strength weighting path.
    """
    ...
    if len(bases_lin) == 1:
        ...
        if dual_gate_bundle is not None:
    ...
    # Dual gate sidecar path: use the locked base-mixer profile embedded in the bundle.
    if dual_gate_bundle is not None:
        if not DUAL_GATE_AVAILABLE or dual_gate_mod is None:
            raise RuntimeError("Dual residual gate module is not available.")
        if pigment_hexes is None:
            raise ValueError("pigment_hexes are required when using dual_gate_bundle.")
        pred = dual_gate_mod.predict_with_bundle(dual_gate_bundle, list(pigment_hexes), list(w))
        return hex_to_linear_rgb(str(pred["pred_hex"]))
...
def predict_lin(
    ...
    return predict_lin_from_bases_lin(
        ...
        nn_model=nn_model,
        nn_scale=nn_scale,
        pigment_hexes=bases_hex,
        dual_gate_bundle=dual_gate_bundle,
    )
...
def inverse_mix_recipe(
    ...
    nn_model: Optional["ResidualNN"],
    nn_scale: float,
    dual_gate_bundle: Optional[Dict[str, Any]],
    max_colors: int,
    max_parts: int,
...
st.caption("Forward mixer (Linear / KM / YN-KM) + optional dual residual gate sidecar + log-poly calibration + optional NN residual + inverse 'Get Mix Recipe' search.")
...
# ============================================================
# Auto-load bundled dual residual gate sidecar (if present)
# ============================================================
BUNDLED_DUAL_GATE_FILENAME = "miniapp_dual_residual_gate_bundle.joblib"
BUNDLED_DUAL_GATE_PATH = os.path.join(APP_DIR, BUNDLED_DUAL_GATE_FILENAME)

if "auto_loaded_dual_gate_bundle" not in st.session_state:
    st.session_state["dual_gate_status"] = {
        "expected_path": BUNDLED_DUAL_GATE_PATH,
        "found": False,
        "loaded": False,
        "error": None,
    }
    try:
        if os.path.exists(BUNDLED_DUAL_GATE_PATH):
            st.session_state["dual_gate_status"]["found"] = True
            if DUAL_GATE_AVAILABLE and dual_gate_mod is not None:
                st.session_state["dual_gate_bundle"] = dual_gate_mod.load_bundle(BUNDLED_DUAL_GATE_PATH)
                st.session_state["dual_gate_status"]["loaded"] = True
            else:
                st.session_state["dual_gate_status"]["error"] = _DUAL_GATE_IMPORT_ERROR or "Dual residual gate module unavailable."
    except Exception as e:
        st.session_state["dual_gate_status"]["error"] = str(e)

    st.session_state["auto_loaded_dual_gate_bundle"] = True

if "use_dual_gate" not in st.session_state:
    st.session_state["use_dual_gate"] = False
...
with st.sidebar:
    ...
    _dg = st.session_state.get("dual_gate_status", {})
    if isinstance(_dg, dict):
        if bool(_dg.get("loaded", False)):
            st.success(f"Auto-loaded dual gate sidecar: {os.path.basename(str(_dg.get('expected_path','')))}")
        elif bool(_dg.get("found", False)) and _dg.get("error"):
            st.warning(f"Dual gate sidecar found but failed to load: {_dg.get('error')}")
        elif bool(_dg.get("found", False)):
            st.info(f"Dual gate sidecar found but not loaded.")
        else:
            st.info(f"No bundled dual gate sidecar found. Expected: {os.path.basename(str(_dg.get('expected_path','')))}")
    ...
    if str(engine).startswith("GeoLogMix"):
        ...
        if str(engine) == "GeoLogMix-3 (XYZ)":
            ...
            with st.expander("Fit GeoLogMix-3 (M + b) from CSV", expanded=False):
                ...
                fitMb_clicked = st.button("Fit (M,b) now", use_container_width=True, key="geolog_fitMb_btn")
                if fitMb_clicked:
    ...
    if st.button("Reset strength params to learned defaults", use_container_width=True):
        st.session_state["strength_enable"] = False
    ...
    st.markdown("---")
    st.header("Dual residual gate sidecar")
    st.caption("Benchmark-tuned post-mix improver for the locked Hybrid (KM ⊕ Linear) profile. When enabled, it bypasses the log-poly calibrator, NN residual, and strength weighting path.")

    if not DUAL_GATE_AVAILABLE:
        st.info("Dual gate module is not available in this build.")
        if _DUAL_GATE_IMPORT_ERROR:
            st.caption(f"Import error: {_DUAL_GATE_IMPORT_ERROR}")
        use_dual_gate = False
    else:
        dg_upload = st.file_uploader("Load sidecar bundle (.joblib)", type=["joblib", "pkl"], key="dual_gate_bundle_file")
        if dg_upload is not None:
            try:
                st.session_state["dual_gate_bundle"] = dual_gate_mod.joblib.load(io.BytesIO(dg_upload.getvalue()))
                st.session_state["dual_gate_status"] = {
                    "expected_path": "<uploaded>",
                    "found": True,
                    "loaded": True,
                    "error": None,
                }
                st.success("Loaded dual gate sidecar bundle.")
            except Exception as e:
                st.error(f"Failed to load dual gate bundle: {e}")

        dg_bundle = st.session_state.get("dual_gate_bundle")
        if isinstance(dg_bundle, dict):
            try:
                bm = dg_bundle.get("base_mixer", {})
                n_rows = int(dg_bundle.get("gate_training_summary", {}).get("n_rows", 0))
                st.caption(
                    f"Bundle type: {dg_bundle.get('model_type', 'unknown')} | "
                    f"rows={n_rows} | known pigments={len(dual_gate_bundle_known_hexes(dg_bundle))} | "
                    f"engine={bm.get('engine', '?')}"
                )
            except Exception:
                pass

        use_dual_gate = st.checkbox(
            "Enable dual residual gate sidecar",
            value=bool(st.session_state.get("use_dual_gate", False)),
            disabled=not isinstance(st.session_state.get("dual_gate_bundle"), dict),
            key="use_dual_gate",
            help="Uses the benchmark-tuned dual residual gate sidecar. This path bypasses calibrator, NN residual, and strength weighting.",
        )
...
# Dual residual gate sidecar: benchmark-tuned for the locked client profile.
dual_gate_active: Optional[Dict[str, Any]] = (
    st.session_state.get("dual_gate_bundle")
    if (DUAL_GATE_AVAILABLE and isinstance(st.session_state.get("dual_gate_bundle"), dict) and st.session_state.get("use_dual_gate", False))
    else None
)
if dual_gate_active is not None:
    mism3: List[str] = []
    bm = dual_gate_active.get("base_mixer", {}) if isinstance(dual_gate_active, dict) else {}
    if str(bm.get("engine", "")) and str(bm.get("engine")) != str(engine):
        mism3.append(f"engine={bm.get('engine')} (bundle) vs {engine} (current)")
    if "ks_eps" in bm and abs(float(bm.get("ks_eps", ks_eps)) - float(ks_eps)) > 1e-12:
        mism3.append(f"KS_EPS={bm.get('ks_eps')} (bundle) vs {ks_eps} (current)")
    if str(engine).startswith("Hybrid") and "hybrid_t" in bm:
        if abs(float(bm.get("hybrid_t", hybrid_t)) - float(hybrid_t)) > 1e-12:
            mism3.append(f"Hybrid t={bm.get('hybrid_t')} (bundle) vs {hybrid_t} (current)")
    if bool(st.session_state.get("strength_enable", False)):
        mism3.append("strength weighting is ON")
    if mism3:
        st.sidebar.warning(
            "Dual gate sidecar settings mismatch → sidecar disabled. "
            "Use the locked client profile (and strength weighting OFF) for this bundle. "
            f"Details: {' | '.join(mism3)}"
        )
        dual_gate_active = None

def _dual_gate_for_hexes_or_none(bundle: Optional[Dict[str, Any]], hexes: Sequence[str], context: str) -> Optional[Dict[str, Any]]:
    if bundle is None:
        return None
    if not dual_gate_allows_hexes(bundle, hexes):
        st.warning(
            f"Dual gate sidecar does not cover all pigments for {context}. "
            f"Sidecar disabled for this {context}."
        )
        return None
    return bundle
...
with inv_right:
    ...
    if st.button("Load preset", use_container_width=True, key="load_preset_btn"):
...
if run_inv:
    try:
        ...
        _dual_for_inv = _dual_gate_for_hexes_or_none(dual_gate_active, palette_hexes, "inverse search")
        if _dual_for_inv is not None and (_cal_for_inv is not None or _nn_for_inv is not None):
            st.info("Dual gate sidecar is ON for inverse search; calibrator and NN residual are bypassed for this run.")
        ...
        with st.spinner("Searching..."):
            sols = inverse_mix_recipe(
                ...
                ks_eps=float(ks_eps),
                yn_n=float(yn_n),
                calibrator=None if _dual_for_inv is not None else _cal_for_inv,
                nn_model=None if _dual_for_inv is not None else _nn_for_inv,
                nn_scale=float(nn_scale),
                dual_gate_bundle=_dual_for_inv,
        ...
        if not sols:
            ...
            if PANDAS_AVAILABLE:
                ...
                st.dataframe(pd.DataFrame(_top_rows), use_container_width=True)
            else:
                st.write(sols)
...
if run_eval:
    if eval_csv is None:
        ...
        try:
            ...
            _dual_for_eval = _dual_gate_for_hexes_or_none(dual_gate_active, _all_hex, "batch evaluation")
            if _dual_for_eval is not None and (_cal_for_eval is not None or _nn_for_eval is not None):
                st.info("Dual gate sidecar is ON for batch evaluation; calibrator and NN residual are bypassed for this run.")
            ...
            df, summary, group = batch_evaluate(
                ...
                ks_eps=float(ks_eps),
                yn_n=float(yn_n),
                calibrator=None if _dual_for_eval is not None else _cal_for_eval,
                nn_model=None if _dual_for_eval is not None else _nn_for_eval,
                nn_scale=float(nn_scale),
                dual_gate_bundle=_dual_for_eval,
            )
            st.markdown("### Summary")
            st.json(summary)
...
with c_reset[0]:
    reset_fwd = st.button("Reset forward editor", key="fwd_reset_editor", use_container_width=True)
if reset_fwd:
...
with st.expander("Paste Trycolors recipe (optional)", expanded=True):
    ...
    with cB:
        apply_paste = st.button("Load pasted recipe", key="fwd_apply_paste", use_container_width=True)
    with cC:
...
for i in range(int(n_bases)):
    ...
    with c3:
        st.image(make_swatch(rows[i]["hex"], size=64), use_container_width=False)
...
with st.expander("Auto-tune KM parameters (diagnostic for this ONE recipe)", expanded=False):
    ...
    if not show_rmse:
        ...
        if tgt0 is None:
            ...
            if st.button("Run auto-tune", key="fwd_autotune", use_container_width=True):
                try:
...
mix_clicked = st.button("Mix (forward)", type="secondary", use_container_width=True)
...
if mix_clicked:
    try:
        ...
        # Stage 0: engine-only prediction (no calibrator / no NN / no sidecar)
        engine_lin = predict_lin(
            ...
            nn_model=None,
            nn_scale=0.0,
            dual_gate_bundle=None,
        )
        engine_hex = linear_rgb_to_hex(engine_lin)

        _cal_for_fwd = None

        # Optional benchmark-tuned sidecar path.
        _dg_for_fwd = _dual_gate_for_hexes_or_none(dual_gate_active, bases_hex, "forward mix")
        dual_gate_info: Optional[Dict[str, Any]] = None
        if _dg_for_fwd is not None:
            dual_gate_info = dual_gate_mod.predict_with_bundle(_dg_for_fwd, list(bases_hex), list(normalize_weights(w))) if (DUAL_GATE_AVAILABLE and dual_gate_mod is not None) else None
            dg_hex = clean_hex(dual_gate_info["pred_hex"]) if isinstance(dual_gate_info, dict) else None
            if dg_hex is None:
                raise RuntimeError("Dual gate sidecar prediction failed.")
            dg_lin = hex_to_linear_rgb(dg_hex)
            cal_lin = engine_lin
            cal_hex = engine_hex
            nn_lin = dg_lin
            nn_hex = dg_hex
            ...
            if _cal_for_fwd is not None:
                ...
                cal_hex = linear_rgb_to_hex(cal_lin)
            else:
                cal_lin = engine_lin
                cal_hex = engine_hex
        ...
        with colS1:
            if _dg_for_fwd is not None:
                show_swatch("+ Dual gate sidecar", dg_hex, size=int(swatch_size))
            else:
        ...
        with colB:
            if show_rmse:
                ...
                if tgt is not None:
                    ...
                    if _dg_for_fwd is not None:
                        st.caption("Dual gate sidecar: ON | Calibrator: BYPASSED | NN residual: BYPASSED | Strength weighting: BYPASSED")
                        if isinstance(dual_gate_info, dict):
                            st.caption(
                                f"Gate choose-k2 prob={float(dual_gate_info.get('gate_choose2_prob', float('nan'))):.3f} | "
                                f"alpha→k2={float(dual_gate_info.get('gate_alpha_to_k2', float('nan'))):.3f} | "
                                f"override={bool(dual_gate_info.get('forced_model3_override', False))}"
                            )
                    else:
                        st.caption(
                    ...
                    if _dg_for_fwd is not None:
                        st.write(f"RMSE (linear RGB) — +dual gate: `{rmse_final:.6f}`")
                    ...
                    try:
                        ...
                        if _dg_for_fwd is not None:
                            st.write(f"ΔE00 — +dual gate: `{de_final:.2f}`  (Match `{match_percent_from_de00(de_final):.1f}%`)")
        ...
        with colC:
            ...
            strength_on = bool(st.session_state.get("strength_enable", False)) and (_dg_for_fwd is None)
            gamma = float(st.session_state.get("strength_gamma", 1.0))
            ...
            if _dg_for_fwd is not None:
                st.caption("Dual gate sidecar uses normalized nominal weights; strength weighting is bypassed.")
            elif strength_on and gamma > 0.0:
                try:
            ...
            if _dg_for_fwd is not None and isinstance(dual_gate_info, dict):
                _tech_lines += [
                    "dual_gate=" + out_hex,
                    "gate_choose2_prob=" + str(dual_gate_info.get("gate_choose2_prob")),
                    "gate_alpha_to_k2=" + str(dual_gate_info.get("gate_alpha_to_k2")),
                    "forced_model3_override=" + str(dual_gate_info.get("forced_model3_override")),
                    "k2_hex=" + str(dual_gate_info.get("k2_hex")),
                    "k3_hex=" + str(dual_gate_info.get("k3_hex")),
                ]
            else:
                _tech_lines += [
                    "calibrated=" + cal_hex,
